...
class Widget:
    ...
    def __init__(self, elem):
        ...
        self.actionTypes = [property_to_event[p] for p in property_to_event if elem.get(p) == "true"]
    ...
    def dump(self):
        ...
        if self.contentDescRelevant:
            ...
            description.append("accessibility information: scroll to see more options(very useful!)")
...
class Oracle:
    ...
    def __init__(self, _rules: dict, _attribute, _constant):
        self.rules = deepcopy(_rules)
    ...
    def verify(self, controller: AndroidController = None, _raw_hierarchy = None) -> bool:
        raw_hierarchy = RawHierarchy(controller.device.dump_hierarchy()) if _raw_hierarchy is None else _raw_hierarchy
        widget: Widget = raw_hierarchy.buildWidget(self.rules)
        if widget is None:
            return False
        if self.type == "equal":
            print(f"{widget.attrib.get(self.attribute)} == {self.constant}?")
            return widget.attrib.get(self.attribute) == self.constant
